        ...
        for obstruction in self.obstructions:
            ...
            if pos[2] >= obstruction_pos[0] and pos[0] <= obstruction_pos[2]:
                if pos[3] >= obstruction_pos[1] and pos[1] <= obstruction_pos[3]:
                    ...
                    if abs(pos[3] - obstruction_pos[1]) < 5:  # Top collision
                        self.y = -abs(self.y)
                    elif abs(pos[1] - obstruction_pos[3]) < 5:  # Bottom collision
                        self.y = abs(self.y)
                    elif abs(pos[2] - obstruction_pos[0]) < 5:  # Left collision
                        self.x = -abs(self.x)
                    elif abs(pos[0] - obstruction_pos[2]) < 5:  # Right collision
                        self.x = abs(self.x)
                    return True
        return False
...
def start_game(event):
    ...
    for i in range(rows):  # Move bricks into position
        for j in range(19):
